...
ID: GroupID,
message: `👋🏻𝐊𝐨𝐧𝐧𝐢𝐜𝐡𝐢𝐰𝐚 𝐄𝐯𝐞𝐫𝐲𝐨𝐧𝐞, ${Timers}
...
userWel.message = `👋🏻𝐊𝐨𝐧𝐧𝐢𝐜𝐡𝐢𝐰𝐚 𝐄𝐯𝐞𝐫𝐲𝐨𝐧𝐞, ${Timers}
...
return;
} else if (BanGroup && !ֆǟӄʊʀǟ.fromMe && !ֆǟӄʊʀǟ.isSenderDev) {
...
Bson.wallpapers[
Math.floor(
Math.random() * Bson.wallpapers.length
)
];
...
try {
var groupDataRank =
await ӄʀǟӄɨռʐ.groupMetadata(
chat.key.remoteJid
);
const media =
await ӄʀǟӄɨռʐ.prepareMessage(
ֆǟӄʊʀǟ.chatId,
fs.readFileSync(
`${աɦօֆɛռȶɦǟȶ.toString()}.jpeg`
),
...
console.log(
Kolor.yellow(
`💡INFO ⬰ New Updates Pending! Auto-Pulling && Updating!`
)
);
})
