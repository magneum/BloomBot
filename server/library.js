...
let commandListCacheTimestamp = 0;
...
module.exports = async (BloomBot, vChat, update, store) => {
  ...
  const updateCommandListCache = () => {
    commandListCache = commandList;
    commandListCacheTimestamp = Date.now();
  };

  const isCommandListCacheValid = () => {
    if (!commandListCache) {
      return false;
    }
    const routesFolderModifiedTime = fs.statSync(routePath).mtimeMs;
    return commandListCacheTimestamp >= routesFolderModifiedTime;
  };
  ...
  const isbotAdmin = vChat.isGroup
    ...
    : false;
  const isAdmin = vChat.isGroup ? groupAdmins.includes(vChat.sender) : false;
  ...
  // Check if the cached command list is still valid
  if (isCommandListCacheValid()) {
    commandList = commandListCache;
  } else {
    ...
    // Update the command list cache
    updateCommandListCache();
  }
