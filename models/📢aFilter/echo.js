...
> _${WhatsBot.prefix}${finalname} reply to Audio_`
    ...
    if (/audio/.test(WhatsBot.mime)) {
      ...
      WhatsBot.exec(
        `${WhatsBot.pathFFmpeg} -i ${media} -af "aecho=0.8:0.9:1000:0.3" ${random}.mp3`,
        async (error) => {
...
> _${voxbot.prefix}${finalname} reply to Audio_`
    ...
    if (/audio/.test(voxbot.mime)) {
      ...
      voxbot.exec(
        `${voxbot.pathFFmpeg} -i ${media} -af "aecho=0.8:0.9:1000:0.3" ${random}.mp3`,
        async (error) => {
          return voxchat.reply(`*🕊️You:* ${
