...
║ Also, Forks won't be entertained.1
...
let ʜᴇɴᴛᴀɪ = await akaneko.nsfw.hentai();
return message
.reply({
embeds: [
new MessageEmbed()
...
.setDescription(
`**\`\`\`fix
Hentais for you perverts to use, however do understand that I'm the only one working on this, and I hand pick images to add, so you may get repeated images!
\`\`\`**`
)
.setImage(ʜᴇɴᴛᴀɪ)
