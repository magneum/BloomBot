...
module.exports = {
  name: `weather`,
  𝓜𝓮𝓮6ʍօʀɛ: `*${ᴋᴇɪ}weather* <name of city>`,
  async handle(ӄʀǟӄɨռʐ, chat, 𝓜𝖎𝖟𝖚ӄ𝖎, Needs, ꜱɪᴛʀᴀᴘ, Clock, Ping) {
    ...
    // ⬡ ™𝐊𝐫𝐚𝐤𝐢𝐧𝐳 ⬡==========================⬡    🍁 (c)爪𝖎𝖟𝖚ӄ𝖎 🍁    ⬡==========================⬡ 𝐋𝐚𝐛™ ⬡
    if (Needs.length === 0 || Needs.length < 1) {
      ...
      Needs[Needs.length - 1] === `tom` ||
      Needs[Needs.length - 1] === `tomorrow`
    ) {
      ...
      https.get(url, function (response) {
        ...
        response.on(`data`, function (data) {
          ...
          const minTempInC = Number(weatherData.list[7].main.temp_min).toFixed(
            2
          );
          const minTempInF = (minTempInC * 1.8 + 32).toFixed(2);
          const maxTempInC = Number(weatherData.list[7].main.temp_max).toFixed(
            2
          );
