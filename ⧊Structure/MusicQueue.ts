...
export class MusicQueue {
  ...
  public stop() {
    ...
    !config.PRUNING &&
      this.textChannel.send("Leaving voice channel...").catch(console.error);
    ...
    setTimeout(() => {
      ...
      !config.PRUNING && this.textChannel.send("Leaving voice channel...");
    }, 100);
  }
    ...
    collector.on("collect", async (reaction, user) => {
      ...
      switch (reaction.emoji.name) {
        ...
        case "🔉":
          ...
          this.textChannel
            .send(
              `<@${user}> 🔉 decreased the volume, the volume is now ${this.volume}%`
            )
            .catch(console.error);
          break;
        ...
        case "🔊":
          ...
          this.textChannel
            .send(
              `<@${user}> 🔊 increased the volume, the volume is now ${this.volume}%`
            )
            .catch(console.error);
          break;
