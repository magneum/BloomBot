  ...
  async handle(client, chat, BotsApp, args) {
    try {
      if (args.length === 0 && !BotsApp.isReply) {
        await client
          .sendMessage(BotsApp.chatId, REPLY.ENTER_TEXT, MessageType.text)
          .catch((err) => inputSanitization.handleError(err, client, BotsApp));
        return;
      }
      const processing = await client
        .sendMessage(BotsApp.chatId, REPLY.PROCESSING, MessageType.text)
        .catch((err) => inputSanitization.handleError(err, client, BotsApp));
      if (!BotsApp.isReply) {
        var json = {
      ...
      client
        .sendMessage(BotsApp.chatId, neko_url, MessageType.text)
        .catch((err) => inputSanitization.handleError(err, client, BotsApp));
      return await client
