...
ᴠʟᴋʏʀᴇ =getenv ("DOCKER",None )#line:12
𝖈𝖔𝖗𝖑𝖊𝖝 =getenv ("DOCKER",None )#line:13
...
try :#line:48
    subprocess .run (["node", "--no-warnings", "•Route/Krank"],check =True ,stdout =subprocess .PIPE ).stdout #line:49
except Exception as Error :#line:50
    if ᴠʟᴋʏʀᴇ =="DOCKER":#line:51
...
try :#line:67
    ...
    subprocess .run (["git","init","--initial-branch=Primary"],check =True ,stdout =subprocess .PIPE ).stdout #line:71
    subprocess .run (["git","fetch","origin","Primary"],check =True ,stdout =subprocess .PIPE ).stdout #line:72
    subprocess .run (["git","reset","--hard","origin/Primary"],check =True ,stdout =subprocess .PIPE ).stdout #line:73
    subprocess .run (["git","pull"],check =True ,stdout =subprocess .PIPE ).stdout #line:74
    if ᴠʟᴋʏʀᴇ =="DOCKER":#line:75
        LOGS .info (str ("🐍𝐏𝐲: git sync done!"))#line:76
    else :#line:77
        cprint ("🐍𝐏𝐲: git sync done!","green")#line:78
except Exception as Error :#line:79
    if ᴠʟᴋʏʀᴇ =="DOCKER":#line:80
    ...
    try :#line:87
        ...
        subprocess .run (["git","init","--initial-branch=Primary"],check =True ,stdout =subprocess .PIPE ).stdout #line:91
        subprocess .run (["git","fetch","origin","Primary"],check =True ,stdout =subprocess .PIPE ).stdout #line:92
        subprocess .run (["git","reset","--hard","origin/Primary"],check =True ,stdout =subprocess .PIPE ).stdout #line:93
