...
module.exports = {
  name: `pfp`,
  commandType: "Fun⚡",
  description: `Get the profile picture of the group in a group conversation or the profile picture of ULTƦON itself in personal chat.`,
  async handle(υℓтяσηℓιєηт, chat, ULTƦON, Arc) {
    try {
      let url = await υℓтяσηℓιєηт.getProfilePicture(ULTƦON.chatId);
      await υℓтяσηℓιєηт
        .sendMessage(
          ULTƦON.chatId,
          {
          ...
          MessageType.image,
          {
            ...
            caption: `Here is the display image. Procured by ULTƦON.`,
            thumbnail: null,
          }
        )
