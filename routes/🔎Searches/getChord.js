...
module.exports = async (Nekobot, Nekos, gmeta, isAdmin, groupName, isbotAdmin, groupAdmins, participants) => {
  try {
    if (!Nekobot.args[0] && isNaN(Nekobot.args[0])) {
...
*⚡Usage* 
> _${Nekobot.prefix}${finalname} Id_
> _get the Id from ${Nekobot.prefix}${finalname}chordlist command!_`
      );
    }

    var data = await Nekobot.axios.get(
      "http://app.chordindonesia.com/?json=get_post&id=" + Nekobot.args[0]
    );
    var clean = (data) => {
      var regex = /(<([^>]+)>)/gi;
