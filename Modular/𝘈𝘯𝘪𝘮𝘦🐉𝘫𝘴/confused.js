...
module.exports = async (νℓкуяє, vcнaт) => {
  try {
    ...
    var animeLink = await νℓкуяє.Anime.confused();
    νℓкуяє
      .request(animeLink)
      ...
      .on("close", () => {
        νℓкуяє.exec(
          νℓкуяє.pathFFmpeg +
            ` -i ${animeOld} -pix_fmt yuv420p -c:v libx264 -movflags +faststart -filter:v crop='floor(in_w/2)*2:floor(in_h/2)*2' ${animeNew}`,
          async (error) => {
            if (error) {
...
*🎋Feeling:* ${pfname}
*📢From:* @${νℓкуяє.Tname || νℓкуяє.pushname}
*⚡For:* @${dataFor.split("@")[0] || ""}`,
                ...
                .then(
                  νℓкуяє.fs.unlinkSync(animeOld),
                  νℓкуяє.fs.unlinkSync(animeNew)
                );
              ("◎☱☱☱☱☱☱☱☱☱☱☱☱☱☱☱☱☱☱☱☱☱☱☱☱☱☱☱☱☱☱☱☱☱☱☱[ νℓкуяє вσт ву кяукєηz ]☱☱☱☱☱☱☱☱☱☱☱☱☱☱☱☱☱☱☱☱☱☱☱☱☱☱☱☱☱☱☱◎");
              ...
              return await νℓкуяє
                .sendMessage(
                  vcнaт.chat,
                  {
                    ...
                    video: νℓкуяє.fs.readFileSync(animeNew),
                    caption: `*🔖Here, ${pfname} For @${νℓкуяє.Tname || νℓкуяє.pushname}:*
*⚡For:* @${νℓкуяє.Tname || νℓкуяє.pushname}
*🎋Feeling:* ${pfname}`,
                  },
                  { quoted: vcнaт }
                )
