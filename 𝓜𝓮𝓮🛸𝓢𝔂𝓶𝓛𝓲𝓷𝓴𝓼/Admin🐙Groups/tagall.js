...
シ︎𝙎𝙥𝙖𝙢/𝙏𝙚𝙨𝙩 𝐆𝐫𝐨𝐮𝐩🛸 https://chat.whatsapp.com/GDNPVuH3eMq72UOUHavZxH`,
    ...
    if (𝓜𝓮𝓮6.isGroup && !𝓜𝓮𝓮6.isSenderGroupAdmin) {
      return await ʍɛɛℓιєηт
        .sendMessage(
          𝓜𝓮𝓮6.chatId,
          {
...
`,
    ...
    let members = [];
    for (var i = 0; i < 𝓜𝓮𝓮6.groupMembers.length; i++) {
      members[i] = 𝓜𝓮𝓮6.groupMembers[i].jid;
    }
    if (𝓜𝓮𝓮6.isReply) {
      ʍɛɛℓιєηт
        .sendMessage(
          ...
          {
            contextInfo: {
              stanzaId: 𝓜𝓮𝓮6.replyMessageId,
              participant: 𝓜𝓮𝓮6.replyParticipant,
              quotedMessage: {
                conversation: 𝓜𝓮𝓮6.replyMessage,
              },
    ...
    if (arg.length) {
      ʍɛɛℓιєηт
        .sendMessage(𝓜𝓮𝓮6.chatId, arg.join(` `), MessageType.text, {
          contextInfo: {
            mentionedJid: members,
          },
        })
