`|🛸|======================================================================================================⬡  𝐊𝐫𝐚𝐤𝐢𝐧𝐳𝐋𝐚𝐛™`;
`|🛸|      (𝐜)𝐂𝐨𝐫𝐭𝐚𝐧𝐚𝐀𝐈 | 𝐢𝐬 𝐚 𝐖𝐡𝐚𝐭𝐬𝐚𝐩𝐩 𝐌𝐮𝐥𝐭𝐢𝐏𝐮𝐫𝐩𝐨𝐬𝐞-𝐔𝐬𝐞𝐫𝐛𝐨𝐭 𝐰𝐢𝐭𝐡 𝐌𝐨𝐝𝐞𝐫𝐚𝐭𝐢𝐨𝐧,𝐀𝐮𝐭𝐨𝐦𝐚𝐭𝐢𝐨𝐧 𝐚𝐧𝐝 𝟏𝟎𝟎+ 𝐦𝐨𝐫𝐞 𝐜𝐨𝐦𝐦𝐚𝐧𝐝𝐬!       |`;
`|🛸|======================================================================================================⬡  𝐊𝐫𝐚𝐤𝐢𝐧𝐳𝐋𝐚𝐛™`;
`|🛸|`;
`|🛸|`;
const fs = require(`fs`);
if (fs.existsSync(`🛸ƈօʀȶǟռǟ™.env`)) {
require(`dotenv`).config({
path: `./🛸ƈօʀȶǟռǟ™.env`,
});
} else {
require(`dotenv`);
}
const env = {
DEVL: process.env.DEVL === undefined ? `No` : process.env.DEVL,
CORTANA: process.env.CORTANA === undefined ? `Null` : process.env.CORTANA,
CORTANAMDB:
process.env.CORTANAMDB === undefined ? `Null` : process.env.CORTANAMDB,
HEROKU_API_KEY:
process.env.HEROKU_API_KEY === undefined
? `Null`
: process.env.HEROKU_API_KEY,
HEROKU_APP_NAME:
process.env.HEROKU_APP_NAME === undefined
? `Null`
: process.env.HEROKU_APP_NAME,
IMDB: process.env.IMDB,
FOXTROT: `^[${process.env.FOXTROT}]`,
CCD: process.env.CCD,
OCR: process.env.OCR,
TEN: process.env.TEN,
WAPI: process.env.WAPI,
MEE: `./Cortana☣️Reactor/ƈօʀȶǟռǟ.png`,
HASH: `https://chat.whatsapp.com/LKN8uVBd8ucHRHofz0jBSd`,
HASHS: `https://chat.whatsapp.com/HKO5WLEZxMe3xWCyiv6vBu`,
HASHSH: `https://chat.whatsapp.com/EIn3YFShFmkJ3fvAqf2rpn`,
Kate:
process.env.Kate === undefined
? `918436686758,917430922909`
: process.env.Kate,
HEROKU: process.env.HEROKU,
PRIVACY: process.env.PRIVACY === undefined ? `public` : process.env.PRIVACY,
};
module.exports = env;
`|🛸|`;
`|🛸|`;
`|🛸|======================================================================================================⬡  𝐊𝐫𝐚𝐤𝐢𝐧𝐳𝐋𝐚𝐛™`;
`|🛸|      (𝐜)𝐂𝐨𝐫𝐭𝐚𝐧𝐚𝐀𝐈 | 𝐢𝐬 𝐚 𝐖𝐡𝐚𝐭𝐬𝐚𝐩𝐩 𝐌𝐮𝐥𝐭𝐢𝐏𝐮𝐫𝐩𝐨𝐬𝐞-𝐔𝐬𝐞𝐫𝐛𝐨𝐭 𝐰𝐢𝐭𝐡 𝐌𝐨𝐝𝐞𝐫𝐚𝐭𝐢𝐨𝐧,𝐀𝐮𝐭𝐨𝐦𝐚𝐭𝐢𝐨𝐧 𝐚𝐧𝐝 𝟏𝟎𝟎+ 𝐦𝐨𝐫𝐞 𝐜𝐨𝐦𝐦𝐚𝐧𝐝𝐬!       |`;
`|🛸|======================================================================================================⬡  𝐊𝐫𝐚𝐤𝐢𝐧𝐳𝐋𝐚𝐛™`;
