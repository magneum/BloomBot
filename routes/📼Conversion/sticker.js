...
> _${OpenBot.prefix}${finalname} reply to Image/Video_`
    ...
    if (/image/.test(OpenBot.mime)) {
      var media = await OpenBot.quoted.download();
      var sticker = new OpenBot.Sticker(media, {
        pack: "magneum",
        author: "by OpenBot",
        type: OpenBot.StickerTypes.FULL,
        categories: ["🤩", "🎉"],
        id: "12345",
        quality: 100,
    ...
    } else if (/video/.test(OpenBot.mime)) {
      if ((OpenBot.quoted.msg || OpenBot.quoted).seconds > 20) {
        await OpenBot.sendMessage(ocID.chat, {
          react: {
            text: "❌",
            key: ocID.key,
