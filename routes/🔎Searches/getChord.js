...
module.exports = async (ѕуηтнσяiα, νℓкhat, update, store) => {
  try {
    if (!ѕуηтнσяiα.args[0] && isNaN(ѕуηтнσяiα.args[0])) {
...
*⚡Usage* 
> _${ѕуηтнσяiα.prefix}${pfname} ID_
> _get the ID from ${ѕуηтнσяiα.prefix}${pfname}chordlist command!_`
      );
    }

    let data = await ѕуηтнσяiα.axios.get(
      "http://app.chordindonesia.com/?json=get_post&id=" + ѕуηтнσяiα.args[0]
    );
    var clean = (data) => {
      let regex = /(<([^>]+)>)/gi;
