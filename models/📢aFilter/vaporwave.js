...
> _${WhatsBot.prefix}${finalname} reply to Audio_`
    ...
    if (/audio/.test(WhatsBot.mime)) {
      ...
      WhatsBot.exec(
        `${WhatsBot.pathFFmpeg} -i ${media} -af"aresample=48000,asetrate=48000*0.8" ${random}.mp3`,
        async (error) => {
...
> _${voxbot.prefix}${finalname} reply to Audio_`
    ...
    if (/audio/.test(voxbot.mime)) {
      ...
      voxbot.exec(
        `${voxbot.pathFFmpeg} -i ${media} -af"aresample=48000,asetrate=48000*0.8" ${random}.mp3`,
        async (error) => {
          return voxchat.reply(`*🕊️You:* ${
