...
# |⬡════════════════════════════════════════════|❝ Ⓒ𝐀𝐫𝐜𝐭𝐢𝐱 ☊ ᴘᴏᴡᴇʀᴇᴅ ʙʏ ᴋʀᴀᴋɪɴᴢʟᴀʙ™ ❞|═══════════════════════════════════════════⬡|
# apt update && apt upgrade -y && aptitude install -y jq git curl wget ffmpeg python3 bpm-tools opus-tools python3-pip python-is-python3 && hash -r && npm install -g n && n install latest && npm install -g npm && hash -r && hash -r && npm install -g n && n install latest && npm install -g npm && hash -r && curl -sL https://deb.nodesource.com/setup_18.x | sudo -E bash - && aptitude install -y nodejs && apt autoremove -y && aptitude install gcc g++ make -y && hash -r && curl https://cli-assets.heroku.com/install.sh | sh && heroku plugins:install heroku-builds && pip install -r кяутєк.ts.txt && npm install --force --save && rm package-lock.json && npm install -g spotify-dl spdl-core && git config --global user.name 'KryKnz' && git config --global user.email 'KryKnz@yandex.com' && git config pull.rebase false
FROM python:latest
...
RUN aaptitude install ffmpeg -y 
...
RUN curl -sL https://deb.nodesource.com/setup_18.x | sudo -E bash - && aptitude install -y nodejs && apt autoremove -y && aptitude install gcc g++ make -y && hash -r
